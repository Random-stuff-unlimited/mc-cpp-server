...
void handleCookieRequest(Packet& packet, Server& server) {
    ...
    try {
        cookieIdentifier = packet.getData().readString(32767); // Max string length
    ...
    g_logger->logNetwork(INFO, "Sent Cookie Response for identifier: " + cookieIdentifier + " (no data)", "Configuration");
