...
void handleLoginAcknowledged(Packet& packet, Server& server) {
	...
	if (!player) {
		// g_logger->logNetwork(ERROR, "Error: No player associated with Login Acknowledged packet",
		// "Login");
	...
	// The client has acknowledged the login success, now we officially transition to Configuration
	// state
	player->setPlayerState(PlayerState::Configuration);

	// g_logger->logNetwork(INFO, "Player " + player->getPlayerName() + " successfully acknowledged
	// login - transitioned to Configuration state", "Login");
