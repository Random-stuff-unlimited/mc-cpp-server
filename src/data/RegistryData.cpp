...
void RegistryData::setRegistryId(const std::string& registryId) { _registry_id = registryId; }

void RegistryData::setEntries(const std::vector<RegistryDataEntry>& entries) { _entries = entries; }

void RegistryData::addEntry(const RegistryDataEntry& entry) { _entries.push_back(entry); }
...
void RegistryData::clearEntries() { _entries.clear(); }

bool RegistryData::removeEntry(const std::string& entryId) {
	auto it = std::find_if(_entries.begin(), _entries.end(), [&entryId](const RegistryDataEntry& entry) { return entry.entry_id == entryId; });
