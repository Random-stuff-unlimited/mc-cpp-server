...
bool deserializeRegistryPacket(const std::vector<uint8_t>& packetData, RegistryData& outRegistry) {
    try {
        ...
        if (packetId != RegistryData::PACKET_ID) {
            if (g_logger) {
                g_logger->logNetwork(ERROR,
                    "Invalid packet ID for registry data: expected " +
                    std::to_string(RegistryData::PACKET_ID) + ", got " +
                    std::to_string(packetId), "RegistryData");
            }
            return false;
        }
        ...
        std::vector<uint8_t> registryData(buffer.getData().begin() + (buffer.getData().size() - remainingBytes),
                                         buffer.getData().end());
...
bool compareRegistries(const RegistryData& reg1, const RegistryData& reg2) {
    ...
    for (size_t i = 0; i < entries1.size(); ++i) {
        ...
        if (entry1.entry_id != entry2.entry_id ||
            entry1.has_data != entry2.has_data) {
            return false;
        }
    }
...
void sendRegistryDataBatch(Packet& packet, Server& server, const std::vector<RegistryData>& registries) {
        ...
        try {
            ...
            if (g_logger) {
                g_logger->logNetwork(INFO,
                                   "Registry packet queued for '" + registry.getRegistryId() +
                                   "' with " + std::to_string(registry.getEntryCount()) +
                                   " entries, size: " + std::to_string(packetData.size()) + " bytes",
                                   "Configuration");
            }
            ...
            if (g_logger) {
                g_logger->logNetwork(ERROR,
                                   "Failed to create packet for registry '" + registry.getRegistryId() +
                                   "': " + std::string(e.what()),
                                   "Configuration");
            }
        }
    ...
    if (g_logger) {
        g_logger->logNetwork(INFO,
                           "Registry batch processing complete: " + std::to_string(successfulPackets) +
                           "/" + std::to_string(registries.size()) + " packets sent",
                           "Configuration");
    }
